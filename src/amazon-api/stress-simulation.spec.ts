...
describe('AmazonApiService Stress Simulation', () => {
  ...
  beforeEach(() => {
    service = new AmazonApiService(mockAuthService, mockConfigService, mockCacheManager);
    jest.clearAllMocks();
  });
  ...
  test('1. SigV4 Validation: Structure Check', async () => {
    ...
    const lastCall = mockedAxios.mock.calls[0][0] as any;
  ...
  test('2. Retry Simulation: 503 Server Error & Signature Regeneration', async () => {
    ...
    const call1Date = (mockedAxios.mock.calls[0][0] as any).headers['X-Amz-Date'];
    const call3Date = (mockedAxios.mock.calls[2][0] as any).headers['X-Amz-Date'];
